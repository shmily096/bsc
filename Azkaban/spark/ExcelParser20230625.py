...
class CommonParser:
    ...
    def collect_pg_and_excle_mapping_upn_check_country_code(self, excel_path,df_excel, upn_key, country_code_value):
        ...
        for index, row in df_merged.iterrows():
            ...
            if pd.isnull(row[pg_colume_name]) or row[pg_colume_name] == "":
                warning_msg =(f"{warning_msg},错误类型:数据库缺失,错误明细:upn={row[upn_key]}在LDS数据库不存在")
                logger.warning(f"Excel file:{exlce_name},错误类型:数据库缺失,错误明细:upn={row[upn_key]}在LDS数据库不存在")
                ...
                warning_msg =(f"{warning_msg}, 错误类型:数据不一致,错误明细:upn={row[upn_key]},excel是{excel_value_2},LDS数据库是{pg_value}")
                logger.warning(f"Excel file:{exlce_name},错误类型:数据不一致,错误明细:upn={row[upn_key]},excel是{excel_value_2},LDS数据库是{pg_value}")
                result_waring_report.append(['WARNING', warning_msg])
    ...
    def save_template(self,output_folder,archive_folder,delivery_csv_folder):
        ...
        total_df.to_excel(output_template,index=False,float_format='%.2f')
...
class Parser525(CommonParser):
    ...
    def extract_info(self):
        ...
        if round(total_amount,2) != round(total_abbr,2):
            raise Exception('Error! Please check amount %s, %s'%(total_abbr,total_amount))
...
class Parser10(CommonParser):
    ...
    def extract_info(self):
       ...
       'Storage conditions', 'Date of Manuf.']
        ...
        #xul12 end
        #xul12 changed
        return extracted_df,delivery_df,total_amount,total_qty,result_waring_report_df
        #xul12 end
