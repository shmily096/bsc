...
sync_inbound_outbound_dn_mapping() {
    sync_data_sqlserver "inbound_outbound_dn_mapping" "SELECT  ID
                                                            ,UpdateDT
                                                            ,Active
                                                            ,InboundDN
                                                            ,OutboundDN
...
sync_inbound_tracking() {
    sync_data_sqlserver "inbound_tracking" "SELECT ID
                        ...
                        FROM TRANS_ShipmentStatusInbound
                        where format(UpdateDT, 'yyyy-MM-dd')='$sync_date'"
}
...
sync_so_invoice_info() {
    sync_data_sqlserver "so_invoice" "SELECT 
                            ...
                            ,SalesType
                        FROM TRANS_Invoice
                        where format(UpdateDT, 'yyyy-MM-dd')>='$start_date'
                         and format(UpdateDT, 'yyyy-MM-dd')<='$sync_date'"
}
