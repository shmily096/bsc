...
# 同步ods_po_dn_cycletime数据
# 同步策略 - 全量
sync_ods_po_dn_cycletime_data() {
    echo "Start syncing ods_otds_supply_week data information"
    sync_data_pg "ods_po_dn_cycletime" "select 
                                        po,
                                        delivery,
                                        deliveryitem,
                                        coalesce(created_datetime,'') created_datetime,
                                        material,
                                        delivery_qty,
                                        coalesce(cycletime_day,0) cycletime_day,
                                        coalesce(china_datetime,'') china_datetime,
                                        coalesce(po_dn_day,0) po_dn_day,
                                        coalesce(po_pgi_day,0) po_pgi_day,
                                        insert_dt
                                        from public.ods_po_dn_cycletime 
                                    where 1=1"

    echo "End syncing ods_po_dn_cycletime data information"
}
...
"leadtime")
    sync_ods_kpi_report_data
    # sync_ods_kpi_category_data
    #sync_ods_kpi_complaint_data  改为SQL server获取
    sync_ods_po_dn_cycletime_data
    ;;
"otds")
    sync_ods_otds_supply_week_data
