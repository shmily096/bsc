...
# 同步DEMO数据
dws_finance_net_sales_gap_data() {
    echo "Start export demo data information"
    #export path table by table
    NEW_export_dir="/bsc/opsdw/export/dws_finance_net_sales_gap"  # HDFS 文件路径
    tablename='dws_finance_net_sales_gap_merge' #PG 处理数据的存储过程  
    #清空全部数据
    demo_export_dir="/bsc/opsdw/export/ads_demo"  # HDFS 文件路径
    demo_pg_funcion="dws_finance_net_sales_gap_del" #PG 删除数据的存储过程    
    export_data $demo_pg_funcion $demo_export_dir
    #插入数据全量更新 暂时是推23年及以后的数据
    export_data_two "$tablename" "$NEW_export_dir" 
    echo "End syncing plant master data information"
...
# 同步DEMO数据
dws_finance_cogs_inventory_revaluation_detail_data() {
    echo "Start export demo data information"
    #export path table by table
    NEW_export_dir="/bsc/opsdw/export/dws_finance_cogs_inventory_revaluation_detail"  # HDFS 文件路径
    tablename='dws_finance_cogs_inventory_revaluation_detail_merge' #PG 处理数据的存储过程  
    #清空全部数据
    demo_export_dir="/bsc/opsdw/export/ads_demo"  # HDFS 文件路径
    demo_pg_funcion="dws_finance_cogs_inventory_revaluation_detail_del" #PG 删除数据的存储过程    
    export_data $demo_pg_funcion $demo_export_dir
    #插入数据全量更新 暂时是推23年及以后的数据
    export_data_two "$tablename" "$NEW_export_dir" 
    echo "End syncing plant master data information"
