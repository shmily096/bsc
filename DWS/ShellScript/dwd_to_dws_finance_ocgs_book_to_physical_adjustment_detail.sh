#!/bin/bash
# Function:
#   sync up dws_finance_ocgs_book_to_physical_adjustment_detail 
# History:
...
echo "start syncing dws_finance_ocgs_book_to_physical_adjustment_detail data into DWS layer on ${sync_date} : ${sync_date[year]}"
...
drop table if exists tmp_dws_finance_ocgs_inventory_charges_detail;
create table  tmp_dws_finance_ocgs_inventory_charges_detail stored as orc as 
	SELECT 
		xx.disvision
		,xx.year_mon
		,xx.value_all
	...
	where items ='COGS_Bk_To_Phys - Book to Physical Adjustment'
		and  years||mon_n<=versions
	group by disvision,years||mon_n,currency
	)xx
	left join (	
		select 
			division_display_name AS disvision,
			year_mon,
			sum(quantity) qty
		from dws_fact_inventory_onhand_by_mon
		WHERE inventory_type='DC' AND plant IN ('D835','D836','D837','D838')
		group by division_display_name,year_mon
	)ee
...
insert overwrite table dws_finance_ocgs_book_to_physical_adjustment_detail partition(year_mon)
SELECT 
		disvision,
		material,
		qty,
...
echo "two $delete_tmp"
$hive -e "$delete_tmp"
echo "End syncing dws_finance_ocgs_book_to_physical_adjustment_detail data into DWS layer on ${sync_date} .................."