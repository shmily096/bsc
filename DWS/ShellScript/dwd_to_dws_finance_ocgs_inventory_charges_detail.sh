#!/bin/bash
# Function:
#   sync up dws_finance_ocgs_inventory_charges_detail 
# History:
...
echo "start syncing dws_finance_ocgs_inventory_charges_detail data into DWS layer on ${sync_date} : ${sync_date[year]}"
...
create table  tmp_dws_finance_ocgs_inventory_charges_detail stored as orc as 
	...
	where items ='InvCharges - Total Inventory Charges'
...
insert overwrite table dws_finance_ocgs_inventory_charges_detail partition(year_mon)
SELECT 
		disvision,
		material,
		qty,
...
echo "two $delete_tmp"
$hive -e "$delete_tmp"
echo "End syncing dws_finance_ocgs_inventory_charges_detail data into DWS layer on ${sync_date} .................."