-- Hive SQL
-- Function：product line cust level 维度leadtime e2e lifecycle
-- 行数据表示：
-- History:
-- 2021-06-21    Donny   v1.0    init

drop table if exists dws_lifecycle_leadtime_SLC_daily_trans;
create external table  dws_lifecycle_leadtime_SLC_daily_trans
(
    pgi_date                                string
    ...
    ,in_store                               float
    ,E2E                                    float
) comment 'leadtime lifecycle SLC'
partitioned by(dt string)
stored as parquet
location '/bsc/opsdw/dws/dws_lifecycle_leadtime_SLC_daily_trans/'
