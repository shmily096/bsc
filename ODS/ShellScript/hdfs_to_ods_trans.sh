...
trans_ctmshipmentstatus_sql="
load data inpath '/bsc/origin_data/$origin_db_name/trans_ctmshipmentstatus/$sync_date' overwrite
into table ${target_db_name}.ods_trans_ctmshipmentstatus
partition(dt='$sync_date');"
#TRANS_WorkOrder_OrdZ
ods_trans_workorder_ordz_sql="
-- Load data into table ods_trans_workorder_ordz from hdfs /bsc/origin_data/bsc_app_ops/TRANS_WorkOrder_OrdZ
load data inpath '/bsc/origin_data/$origin_db_name/TRANS_WorkOrder_OrdZ/$sync_date' overwrite
into table ${target_db_name}.ods_trans_workorder_ordz
partition(dt='$sync_date');"
...
if [ "$1"x = "MDM_DealerMaster"x ];then
	...
	echo "$1 finish"
elif [ "$1"x = "trans_ctmshipmentstatus"x ];then
	echo "$1 $trans_ctmshipmentstatus_sql"
	$hive -e "$trans_ctmshipmentstatus_sql"
	echo "$1 finish"
elif [ "$1"x = "TRANS_WorkOrder_OrdZ"x ];then
	echo "$1 $ods_trans_workorder_ordz_sql"
	$hive -e "$ods_trans_workorder_ordz_sql"
	echo "$1 finish"
elif [ "$1"x = "TRANS_InventoryTransactions"x ];then
	echo "$1 $mov_sql"
	$hive -e "$mov_sql"
	echo "$1 finish"
elif [ "$1"x = "putaway"x ];then
	echo "$1 $putawy_sql"
	$hive -e "$putawy_sql"
	echo "$1 finish"
elif [ "$1"x = "so_invoice"x ];then
	echo "$1 $so_invoice_sql"
	$hive -e "$so_invoice_sql"
	echo "$1 finish"
elif [ "$1"x = "DutybyUPN"x ];then
	echo "$1 $DutybyUPN_sql"
	$hive -e "$DutybyUPN_sql"
	echo "$1 finish"
else
    $hive -e "$trans_sql"
