#!/bin/bash
# Function:
#   sync up lifecycle_leadtime_division_slcyh data to dwt layer
# History:
# 2021-07-08    Donny   v1.0    init

# 参数
target_db_name='opsdw' # 目标数据库名称
hive=/opt/module/hive3/bin/hive  # Hive的配置路径
hadoop=/opt/module/hadoop3/bin/hadoop # Hadoop的配置路径

# 默认取当前时间的前一天 
if [ -n "$1" ] ;then 
    sync_date=$1
else
    sync_date=$(date  +%F)
fi

declare -A sync_date=$(date +'([day]=%F [year]=%Y [month]=%m)')
yesterday=$(date  +%F)

echo "start syncing data into dws layer on ${sync_date[year]} :${sync_date[month]} .................."

dwt_sql="
use ${target_db_name};
set mapreduce.job.queuename=default;
set hive.exec.dynamic.partition=true;
set hive.exec.dynamic.partition.mode=nonstrict;
set hive.exec.max.dynamic.partitions.pernode=100000;
set hive.exec.max.dynamic.partitions=100000;
set hive.exec.reducers.max=8;
set mapred.reduce.tasks=8;
set hive.exec.parallel=false;


with a as(
    select  year(pgi_date)                                                                  as pgi_year
        ,month(pgi_date)                                                                    as pgi_month
        ,division
        ,percentile_approx(e2e,0.5)                                                         as e2e
        ,percentile_approx(in_store,0.5)                                                    as in_store
        ,percentile_approx((so_order_processing + so_pgi_processing + so_delivery),0.5)     as so_proce
        ,date_format(pgi_date, 'yyyy') as dt_year
        ,date_format(pgi_date, 'MM') as dt_month
    from dws_lifecycle_leadtime_slc_daily_trans
         where in_store is not null
         and e2e is not null
         and so_order_processing is not null
         and so_pgi_processing is not null
--         and so_delivery is not null
         and date_format(pgi_date, 'MM')='${sync_date[month]}'
         and date_format(pgi_date, 'yyyy')= '${sync_date[year]}'
    group by  year(pgi_date)
            ,month(pgi_date)
            ,division
            ,date_format(pgi_date, 'yyyy')
            ,date_format(pgi_date, 'MM')
) 
,b as(
    select  year(pgi_date)                                                                 as pgi_year
        ,month(pgi_date)                                                                   as pgi_month
        ,division
        ,percentile_approx(e2e,0.5)                                                        as e2e
        ,percentile_approx(in_store,0.5)                                                   as in_store
        ,percentile_approx((so_order_processing + so_pgi_processing + so_delivery),0.5)    as so_proce
        ,date_format(pgi_date, 'yyyy') as dt_year
        ,date_format(pgi_date, 'MM') as dt_month
    from dws_lifecycle_leadtime_yh_daily_trans
    where in_store is not null
--and e2e is not null
          and so_order_processing is not null
          and so_pgi_processing is not null
          and so_delivery is not null
          and date_format(pgi_date, 'MM')='${sync_date[month]}'
          and date_format(pgi_date, 'yyyy')= '${sync_date[year]}'
    group by  year(pgi_date)
            ,month(pgi_date)
            ,division
            ,date_format(pgi_date, 'MM')
            ,date_format(pgi_date, 'yyyy')
)

insert overwrite table dwt_lifecycle_leadtime_division_slcyh_topic partition(dt_year, dt_month)
select  a.pgi_year
       ,a.pgi_month
       ,a.division
       ,a.e2e
       ,a.in_store
       ,a.so_proce
       ,'slc' as putaway_location
       ,a.dt_year
       ,a.dt_month
from a 
union all
select  b.pgi_year
       ,b.pgi_month
       ,b.division
       ,b.e2e
       ,b.in_store
       ,b.so_proce
       ,'yh' as putaway_location
       ,b.dt_year
       ,b.dt_month
from b 
;
"
# 2. 执行加载数据SQL
$hive -e "$dwt_sql"

echo "End syncing lifecycle_leadtime_division_slcyh data into DWT layer on $sync_date .................."