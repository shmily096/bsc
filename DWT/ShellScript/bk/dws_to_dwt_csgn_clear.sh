...
from 
(
select so_no ,order_type ,SUBSTR(chinese_socreatedt,0,10) createdt ,material,line_number ,batch ,qty ,
...
join (select so_no ,customer_shipto
from ods_salesorder_partner
where customer_function ='WE') p 
on lpad(a.so_no, 10, '0') =p.so_no 
join tmp_dwd_dim_customer b
on p.customer_shipto=lpad(b.cust_account, 10, '0')
