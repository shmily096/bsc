...
year_month_date<date(current_date)
) aa 
join (
...
date_format(a.d1,'yyyy-MM') as year_mon
from (
select d1 ,division_id ,sum_qty,total_qty,'Supply Constrain' bo_reason from opsdw.tmp_dwd_otds_order_info
union 
select d1 ,division_id ,sum_qty,total_qty,'Demand management' bo_reason from opsdw.tmp_dwd_otds_order_info
union
select d1 ,division_id ,sum_qty,total_qty,'Order management' bo_reason from opsdw.tmp_dwd_otds_order_info
) a 
left join 
(
select 
...
where a.d1 <=(select MAX(pardate)  from opsdw.tmp_dwd_otds_itemlevel);
