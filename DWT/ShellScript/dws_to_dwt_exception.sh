...
ods_dq_maex_bsc_maxdt=`hdfs dfs -ls /bsc/opsdw/ods/ods_dq_maex_bsc | tail -n 1 | awk -F'=' '{print $NF}'`
ods_dq_maex_crm_maxdt=`hdfs dfs -ls /bsc/opsdw/ods/ods_dq_maex_crm | tail -n 1 | awk -F'=' '{print $NF}'`
ods_dq_mvke_maxdt=`hdfs dfs -ls /bsc/opsdw/ods/ods_dq_mvke | tail -n 1 | awk -F'=' '{print $NF}'`
ods_dq_zv001_maxdt=`hdfs dfs -ls /bsc/opsdw/ods/ods_dq_zv001 | tail -n 1 | awk -F'=' '{print $NF}'`
ods_dq_zv002_maxdt=`hdfs dfs -ls /bsc/opsdw/ods/ods_dq_zv002 | tail -n 1 | awk -F'=' '{print $NF}'`
ods_dq_marc_maxdt=`hdfs dfs -ls /bsc/opsdw/ods/ods_dq_marc | tail -n 1 | awk -F'=' '{print $NF}'`
...
drop table if exists tmp_dwd_dim_material_crm;
create table  tmp_dwd_dim_material_crm stored as orc as 
select DISTINCT material material_code from opsdw.ods_dq_maex_crm
where dt='$ods_dq_maex_crm_maxdt' and pr_rel_status ='MR'and country ='CN';

drop table if exists tmp_dwd_dim_material_nomr_crm;
create table  tmp_dwd_dim_material_nomr_crm stored as orc as 
select DISTINCT material material_code from opsdw.ods_dq_maex_crm
where dt='$ods_dq_maex_crm_maxdt' and pr_rel_status <>'MR'and country ='CN';

drop table if exists tmp_ods_dq_mvke;
create table  tmp_ods_dq_mvke stored as orc as
select material ,sorg ,plnt,product_hierarchy,st,mg1,mg2,mg3,mg4 ,dt  
from opsdw.ods_dq_mvke 
where dt='$ods_dq_mvke_maxdt'; 

drop table if exists tmp_ods_dq_zv001;
create table  tmp_ods_dq_zv001 stored as orc as
select DISTINCT material  from opsdw.ods_dq_zv001
where dt='$ods_dq_zv001_maxdt'; 

drop table if exists tmp_ods_dq_zv002;
create table  tmp_ods_dq_zv002 stored as orc as 
select profit_ctr ,product_hierarchy ,profit_ctr ||product_hierarchy value
from opsdw.ods_dq_zv002
where dt='$ods_dq_zv002_maxdt';

drop table if exists tmp_materialmaster_marc;
create table  tmp_materialmaster_marc stored as orc as 
select material ,plant 
,profitcenter 
,esloc storagelocationep
,ms psmatlstatus
,sourcelist
,lgrp  loadinggroup 
,sptc specproctype 
,i posttoinspstk
,lhg 
,case when LENGTH(material)=10 and SUBSTRING(material ,1,1)='6' 
and SUBSTRING(material ,7,1)='-' and SUBSTRING(profitcenter,1,1)='5' then 'CRM'
else 'BSC' end division_type,dt
from opsdw.ods_dq_marc
where dt='$ods_dq_marc_maxdt' and length(plant)=4 and plant in ('D835','D836','D837','D838') 
and !(SUBSTRING(profitcenter,1,1)='5' and SUBSTRING(material ,1,1)!='6');
...
'E-A-MMSTA_VMSTA-01' exception_code,a.dt
...
where a.storagelocationep ='ZEOL' and a.psmatlstatus ='88' and c.sorg='CN10' --4918
union all
select 
a.material,profitcenter,plant,division_type,psmatlstatus para1,c.st para2,
case 
...
on a.material =b.material_code
LEFT JOIN tmp_ods_dq_mvke c on a.material =c.material
where a.storagelocationep ='ZEOL' and a.psmatlstatus ='88' and c.sorg='CN50'
union all 
select 
a.material,profitcenter,plant,division_type,psmatlstatus para1,c.st para2,
case  
...
'E-A-KORDB-02' exception_code,dt
from tmp_materialmaster_marc where plant='D838'  --p14 D838 SLI 异常
union ALL 
select 
material,profitcenter,plant,division_type,SourceList para1,'' para2,
case when a.SourceList is null or a.SourceList='' then 'error' else 'pass' end  exception_type,
'W-C-EMBGR_KORDB-02' exception_code,dt
from tmp_materialmaster_marc a 
join tmp_dwd_dim_material_noapproved b 
on a.material=b.material_code
...
'W-C-EMBGR_KORDB-01' exception_code,dt
from tmp_materialmaster_marc a 
join tmp_dwd_dim_material b 
on a.material=b.material_code
...
material,profitcenter,plant,division_type,SourceList para1,'' para2,
case when SourceList ='X' then 'error' else 'pass' end  exception_type,
'E-A-KORDB-01' exception_code,dt
...
'marc' db_type,
dt
from tmp_dwd_marc_exception where exception_type<>'pass';
