...
select 
DISTINCT  a.so_no ,a.material,a.delivery_id ,a.d2,p.po_dn_day,
...
from ( select so_no ,material,delivery_id,delivery_id||line_number deliveryitem ,MAX(chinese_dncreatedt)  d2
...
union all 
select '2024' h_year, '2024-10-01' h_startDT,'2024-10-01' h_endDT , 7 h_day, 'National Day' h_name
union all
select '2025' h_year, '2025-01-28' h_startDT,'2025-02-04' h_endDT , 8 h_day, 'Chinese New Year' h_name;

drop table if exists tmp_dwd_po_dn_info;
create table  tmp_dwd_po_dn_info stored as orc as 
 select so_no,delivery_id, division_id,year_month_date dt,
...
from (
select a.so_no,b.delivery_id,a.division_id,a.d1 ,b.d2,b.po_dn_day ,b.year_month_date,
...
CAST(COUNT(DISTINCT delivery_id) as decimal(18,6)) as flag3,
'OC0101' kpi_code,
